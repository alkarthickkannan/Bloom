...
import styles from './EdmsProjectImageDetailsWebPart.module.scss';
import * as strings from 'EdmsProjectImageDetailsWebPartStrings';
...
export interface IEdmsProjectImageDetailsWebPartProps {
  description: string;
}

export default class EdmsProjectImageDetailsWebPart extends BaseClientSideWebPart<IEdmsProjectImageDetailsWebPartProps> {

  ImageAvail:boolean = false;
  public render(): void {

    var siteURL = this.context.pageContext.web.absoluteUrl;
    var siteProjectURL = this.context.pageContext.site.absoluteUrl;
    SPComponentLoader.loadCss( siteProjectURL +"/_catalogs/masterpage/BloomHomepage/css/jssorCss.css");
    SPComponentLoader.loadScript(siteProjectURL + "/_catalogs/masterpage/BloomHomepage/js/jquery.min.js");
    SPComponentLoader.loadScript(siteProjectURL + "/_catalogs/masterpage/BloomHomepage/js/jssor.slider.min.js");
    ...
    "<div class='title-section'>" +
      "<div class='button-field'>" +
        "<a href='ImageGallery.aspx' class='pointer' title='Close' style='background:#53545E;'><img src='" + siteProjectURL + "/_catalogs/masterpage/BloomHomepage/images/close-icon.png'>Close</a>" +
      "</div>" +
      "<h2>Image Gallery</h2>" +
    "</div>" +
    `<br>
    ...
    <div data-u='loading' class='jssorl-009-spin' style='position:absolute;top:0px;left:0px;width:100%;height:100%;text-align:center;background-color:rgba(0,0,0,0.7);'>
        <img style='margin-top:-19px;position:relative;top:50%;width:38px;height:38px;' src='"+this.context.pageContext.legacyPageContext.webAbsoluteUrl+"/_catalogs/masterpage/BloomHomepage/images/slider-loader.svg'/>
    </div>
  ...
  async ImgGalDetails(imgeventid, ImgHtml) {
    ...
    for (var i = 0; i < picItemsLen; i++) {
      if (picItems[i].FileSystemObjectType == 0) {
        let actFolderName = picItems[i].FileDirRef;
        let urlFolderName = actFolderName.substr(actFolderName.lastIndexOf('/') + 1);
        if (urlFolderName == imgeventid) {
