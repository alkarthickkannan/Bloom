...
// import 'jquery';
import styles from './EdmsDepartmentTilesWebPart.module.scss';
import * as strings from 'EdmsDepartmentTilesWebPartStrings';
...
export interface IEdmsDepartmentTilesWebPartProps {
  description: string;
}

export default class EdmsDepartmentTilesWebPart extends BaseClientSideWebPart<IEdmsDepartmentTilesWebPartProps> {
  
  public render(): void {
    // SPComponentLoader.loadScript(this.context.pageContext.site.absoluteUrl + "/_catalogs/masterpage/BloomProject/js/jquery-3.3.1.min.js");
    // SPComponentLoader.loadScript("https://zsl85.sharepoint.com/sites/BloomHolding/SiteAssets/JS/slick.js");
    this.domElement.innerHTML = `
    <section class="gallery-section slider" id="gallery-sec">
  ...
  async FetchItems() {
    ...
    let GetListItems = await readItems(listName, columnArray, 4, "ID", "Username", Username);
    var HTML = "";
    var Tiles = [];
    var FavIcon;
    if (GetListItems.length > 0) {
      for (var i = 0; i < GetListItems.length; i++) {
        ...
        if (i == 0) {
          HTML += '<div class="slide col-sm-3 col-xs-12 pad-left0">' +
        ...
        else if(i==3)
        {
          ...
          HTML += '<div class="slide col-sm-3 col-xs-12">' +
            '<div class="img-gallery"><div class="exp-img"><div align="center" class="small-icon"> <img src="'+FavIcon+'"></div></div> <a target="_blank" class="head-h3" href="' + GetListItems[i].LinkURL.Url + '" id="' + GetListItems[i].ID + '" >' + GetListItems[i].Title + '</a> <a class="icon-more TileEdit" style="cursor: pointer;" id="' + GetListItems[i].ID + '" data-toggle="modal" data-target="#Addmodal"></a></div>' +
            //'<div class="img-gallery"> <img src="images/announce-listimg1.jpg"> <a href="#" class="TileEdit" id="'+GetListItems[i].ID+'" >'+GetListItems[i].Title+'<i class="icon-more" data-toggle="modal" data-target="#Addmodal"></i></a> </div>'+
            '</div>';
        }
    ...
    {
        HTML += '<div class="slide col-sm-3 col-xs-12 over-right">' +
    ...
    // SPComponentLoader.loadScript(this.context.pageContext.site.absoluteUrl + "/_catalogs/masterpage/BloomProject/js/jquery-3.3.1.min.js");
    //SPComponentLoader.loadScript(this.context.pageContext.site.absoluteUrl + "/_catalogs/masterpage/BloomProject/js/wow.min.js");
    // SPComponentLoader.loadScript("https://zsl85.sharepoint.com/sites/BloomHolding/SiteAssets/JS/main.js");
  ...
  Validation() {
    ...
    if (!$('#txtTitle').val()) {
      ...
      return false;
    }else if (!regexp.test($('#txtLinkUrl').val().trim())) {
      //$('#txtHyper').focus();
      alertify.set('notifier', 'position', 'top-right');
