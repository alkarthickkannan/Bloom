...
import * as strings from 'EdmsDepartmentMediaGalleryWebPartStrings';
import { readItems, checkUserinGroup } from '../../commonJS';

export interface IEdmsDepartmentMediaGalleryWebPartProps {
  description: string;
}

export default class EdmsDepartmentMediaGalleryWebPart extends BaseClientSideWebPart<IEdmsDepartmentMediaGalleryWebPartProps> {
  ...
  public MediaGallery() {
    ...
    <section class="banner-section ban-sec1">
      <div class="ban-section">
        <h3 class="tt-head" id="HeadMediaGallery"><a id="addEvents" class="pull-right" href="../Pages/ListView.aspx?CName=Media Gallery"> More </a></h3>
    ...
    $('#HeadMediaGallery').prepend(sitename);
  ...
  async GetmediaGalleryItems(userflag) {
    ...
    // IF DISPLAYED ITEMS = 1
      ...
      if (objResults[0].MediaFileType == "Image") {
        ...
        renderHtml += "<div class='item active'>" +
          "<a href='"+ objResults[0].LinkURL.Url +"' https://www.bloomholding.com><img href='"+ objResults[0].LinkURL.Url +"' src='" + objResults[0].Image.Url + "' alt='Slide' title='" + objResults[0].Title + "' /></a>" +
          "</div>"
      }
    }
