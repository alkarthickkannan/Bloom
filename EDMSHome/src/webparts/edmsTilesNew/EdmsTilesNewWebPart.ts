...
import styles from './EdmsTilesNewWebPart.module.scss';
import pnp from 'sp-pnp-js';
import 'jquery';
import * as strings from 'EdmsTilesNewWebPartStrings';
...
// import '../../ExternalRef/css/owl.carousel.css';
// import '../../ExternalRef/css/owl.carousel.min.css';

// require('../../ExternalRef/js/owl.carousel.js');
// require('../../ExternalRef/js/owl.carousel.min.js');
var arrDocument = [];
export interface IEdmsTilesNewWebPartProps {
  description: string;
}

export default class EdmsTilesNewWebPart extends BaseClientSideWebPart<IEdmsTilesNewWebPartProps> {
    ...
    <div class="modal-dialog modal-md">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <input type='file' id='uploadDocFile' name='file' accept='.doc,.docx,.xls,.ppt,.pdf' multiple='' class='file'>
          ...
          <div class="modal-footer">
            <div class="col-xs-12 form-element"> 
            <a id="btnDocAddSubmit" href="#" class="s-button">Submit</a>
            </div>
          </div>
        </div>
      </div>
  ...
  async getDocuments(){
    ...
    pnp.sp.site.getDocumentLibraries(this.context.pageContext.web.absoluteUrl).then(function(data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        // })
        if(data[i].Title != 'Documents')
        {
          strTitle += "<option id='"+data[i].Title+"'>"+data[i].Title+"</option>";  
        }
      }
        ...
        for(var i=0; i<Columndata.d.results.length; i++)
        {
          if(Columndata.d.results[i].Title != "Name")
          {
            DymHtml += '<div class="col-xs-12 form-element">'+
                        '<label>'+Columndata.d.results[i].Title+'</label>'+
  ...
  public renderhtml(objResults) {
    ...
    renderhtml += "<div align='center'>" +
      ...
      "<p class='p-space'><i class='icon-mail'></i>" + objResults["WorkEmail"] + "</p>" +
